...
cls
echo =============================================================
echo               BUSCANDO ACTUALIZACIONES DE MANTENIX
echo =============================================================
echo.
setlocal
...
if errorlevel 1 (echo [ERROR] No se detecto una conexion a internet. & goto EndUpdateCheck)
...
if not exist "%tempVersionFile%" (echo [ERROR] No se pudo obtener el archivo de version desde GitHub. & goto EndUpdateCheck)
...
if not errorlevel 1 (echo [OK]   Ya tienes la ultima version o una superior. & goto EndUpdateCheck)
echo [!] Se encontro una nueva version mas reciente (%latestVersion%)!
set /p "doUpdate=Deseas descargar la nueva version ahora? (S/N): "
if /i not "%doUpdate%"=="S" (echo [INFO] Descarga cancelada por el usuario. & goto EndUpdateCheck)
cls
echo =============================================================
...
echo. & echo        Por favor, espera...
powershell -NoProfile -ExecutionPolicy Bypass -Command "try { (New-Object System.Net.WebClient).DownloadFile('%releaseURL%', '%downloadPath%') } catch {}"
if not exist "%downloadPath%" (echo [ERROR] La descarga ha fallado. Revisa tu conexion o el antivirus. & goto EndUpdateCheck)
echo. & echo [OK]   ¡Descarga completada! & echo.
echo        ========================= INSTRUCCIONES =========================
echo        La nueva version '%fileName%' se ha guardado en tu carpeta de Descargas.
echo.
echo        Para actualizar, por favor cierra este programa y reemplaza
echo        manualmente el archivo antiguo con el que acabas de descargar.
echo        =================================================================
echo.
echo [INFO] Abriendo la carpeta de Descargas para ti...
timeout /t 3 /nobreak >nul
explorer "%USERPROFILE%\Downloads"
:EndUpdateCheck
del "%tempVersionFile%" >nul 2>&1
echo. & echo Presiona una tecla para volver al menu principal... & pause >nul & goto MENU
